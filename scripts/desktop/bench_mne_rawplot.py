            ...
            if hasattr(ax, "set_xlim"):
                ...
                if hasattr(ax, "get_xlabel"):
                    try:
                        ...
                        label = ""
                if not label or "time" in label.lower():
                    try:
                        ax.set_xlim((tmin, tmax))
                        canvas = getattr(plotter, "canvas", None)
                        if canvas is not None and hasattr(canvas, "draw_idle"):
                            canvas.draw_idle()
                        elif canvas is not None and hasattr(canvas, "draw"):
                            canvas.draw()
                        return True
                    except Exception:
                        continue
...
def main() -> None:
    ...
    for idx, (x0, x1) in enumerate(ranges):
        ...
        if not set_time_range(plotter, x0, x1):
            detail = "RawPlotter does not expose a time range setter"
