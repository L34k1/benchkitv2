...
def main() -> None:
    ...
    for fmt in formats:
        ...
        for tool_id in tool_sel.get(fmt, []):
            ...
            if fmt == FMT_EDF and tool_id == TOOL_IO_NEO:
                if importlib.util.find_spec("neo") is None:
                    skip_reason = "SKIP_UNSUPPORTED_DEP"

            if skip_reason:
                summary = summarize_latency_ms([])
                summary.update({"format": fmt, "tool_id": tool_id, "bench_id": BENCH_IO, "status": skip_reason})
                write_json(out / "summary.json", summary)
                write_json(out / "per_file.json", {"rows": [], "status": skip_reason})
                continue
                ...
                for path in files:
                    try:
                        if fmt == FMT_EDF and tool_id == TOOL_IO_PYEDFLIB:
                            ...
                            dt = _load_nwb_h5py(
                                path, args.load_start_s, args.window_s, args.n_ch, args.nwb_dataset_path
                            )

                        else:
                            continue
                    except ModuleNotFoundError as exc:
                        if tool_id == TOOL_IO_NEO and exc.name == "neo":
                            skip_reason = "SKIP_UNSUPPORTED_DEP"
                            break
                        raise
                    except AttributeError:
                        if tool_id == TOOL_IO_NEO:
                            skip_reason = "SKIP_UNSUPPORTED_API"
                            break
                        raise
