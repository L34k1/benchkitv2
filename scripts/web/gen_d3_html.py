...
function yDomain() {{
  return [PAYLOAD.y_min, PAYLOAD.y_max];
}}
...
def main() -> None:
    ...
    p.add_argument("--keep-last-frame", action="store_true", help="Render final frame for visual inspection.")
    args = p.parse_args()
    ...
    if y.size:
        ...
        if finite.any():
            y_min = float(y[finite].min())
            y_max = float(y[finite].max())
        else:
            y_min = -1.0
            y_max = 1.0
    else:
