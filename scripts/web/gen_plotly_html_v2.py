...
from benchkit.common import ensure_dir, out_dir, write_json, env_info
...
def _load_plotly_bundle() -> str | None:
    try:
        import importlib.resources as resources

        with resources.files("plotly").joinpath("package_data/plotly.min.js").open(
            "r", encoding="utf-8"
        ) as f:
            return f.read()
    except Exception:
...
def make_traces(t: np.ndarray, d: np.ndarray, renderer: str) -> List[Dict[str, Any]]:
...
def html_template(payload: Dict[str, Any], plotly_script_tag: str) -> str:
...
def _plotly_script_tag(out_dir: Path) -> str:
    plotly_bundle = _load_plotly_bundle()
    if plotly_bundle:
        js_path = out_dir / "plotly.min.js"
        js_path.write_text(plotly_bundle, encoding="utf-8")
        return f'<script src="{js_path.name}"></script>'
...
def collect_bench_json(html_path: Path, out_json: Path) -> dict:
    ...
    subprocess.run(
        [sys.executable, str(script), "--html", str(html_path), "--out", str(out_json), "--headed"],
        check=True,
    )
...
def main() -> None:
    ...
    p.add_argument(
        "--renderer",
        choices=["scattergl", "scatter"],
        default=os.environ.get("BENCHKIT_PLOTLY_RENDERER", "scattergl"),
        help="Plotly trace renderer (default: scattergl).",
    )
    ...
    write_manifest_contract(
        ...
        sequence=args.sequence if args.bench_id in (BENCH_A1, BENCH_A2) else None,
    ...
    payload: Dict[str, Any] = {
        ...
        "n_ch": int(args.n_ch),
        "traces": make_traces(t, d, args.renderer),
    ...
    plotly_script_tag = _plotly_script_tag(out)
    html = html_template(payload, plotly_script_tag)
    ...
    if args.bench_id == BENCH_TFFR:
        tffr_ms = float(bench_json.get("first_render_ms") or bench_json.get("tffr_ms", float("nan")))
    ...
    steps_rows = steps_from_latencies(lat_ms, steps_target=int(payload["steps"]))
