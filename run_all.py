...
def collect_bench_json(
    html_path: Path,
    out_path: Path,
    log_path: Path,
    console_json_path: Path,
    console_csv_path: Path,
    timeout_ms: int,
) -> None:
    cmd = [sys.executable, str(REPO_ROOT / "scripts" / "web" / "collect_console_playwright.py"),
           "--html", str(html_path),
           "--out", str(out_path),
           "--console-json", str(console_json_path),
           "--console-csv", str(console_csv_path),
...
def run_job(job: Job, log_path: Path, logger: logging.Logger, force: bool) -> Tuple[str, Optional[str], Optional[str], float]:
    ...
    if job.tool_id in {TOOL_PLOTLY, TOOL_D3}:
        try:
            html_name = {
                (TOOL_PLOTLY, BENCH_TFFR): "plotly_bench.html",
                (TOOL_PLOTLY, BENCH_A1): "plotly_bench.html",
                (TOOL_PLOTLY, BENCH_A2): "plotly_bench.html",
            ...
            console_json_path = job.out_dir / "console.json"
            console_csv_path = job.out_dir / "console.csv"
            collect_bench_json(
                html_path,
                bench_json_path,
                log_path,
            ...
            if job.bench_id in (BENCH_A1, BENCH_A2):
                ...
                if job.tool_id == TOOL_PLOTLY and "results" in bench_json:
                    for result in bench_json.get("results", []):
                        ...
                        for row in result.get("rows", []):
                            ...
                            rows.append(out_row)
                elif job.tool_id in {TOOL_D3, TOOL_PLOTLY}:
